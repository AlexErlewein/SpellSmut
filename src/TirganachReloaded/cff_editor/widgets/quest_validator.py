...
class QuestValidator:
    ...
    def set_existing_quests(self, quest_data: Dict[int, Dict]):
        ...
        self.existing_quest_names = set(
            quest.get("name", "") for quest in quest_data.values() if quest.get("name")
        )
    ...
    def validate_quest_detailed(self, quest: EnhancedQuestData) -> ValidationResult:
        ...
        # Dialogue validation
        if quest.dialogues:
            dialogue_errors, dialogue_warnings = self._validate_dialogues(
                quest.dialogues
            )
    ...
    def _validate_basic_info(self, quest: EnhancedQuestData) -> Tuple[List, List, List]:
        ...
        if quest.quest_id is None:
            ...
            self.custom_quest_id_range[0]
            <= quest.quest_id
            <= self.custom_quest_id_range[1]
            and quest.quest_id in self.existing_quest_ids
        ):
            errors.append(
                ValidationError(
                    level="error",
    ...
    def _validate_dialogues(self, dialogues: List[Dialogue]) -> Tuple[List, List]:
        ...
        for i, dialogue in enumerate(dialogues):
            ...
            # Note: Speaker validation removed as Dialogue class doesn't have speaker attribute
            # Speaker information is handled at the dialogue node level, not quest level
            pass

        # Check dialogue flow
        # Note: Speaker-based dialogue counting removed as Dialogue class doesn't have speaker attribute
        npc_dialogues = []
        player_dialogues = []
        ...
        duplicate_texts = [
            text for text in dialogue_texts if dialogue_texts.count(text) > 1
        ]
        if duplicate_texts:
            warnings.append(
                ValidationError(
    ...
    def _validate_cff_compatibility(
        self, quest: EnhancedQuestData
    ) -> Tuple[List, List]:
    ...
    def _validate_lua_compatibility(
        self, quest: EnhancedQuestData
    ) -> Tuple[List, List]:
        ...
        if quest.dialogues:
            for i, dialogue in enumerate(quest.dialogues):
                if dialogue.text and any(
                    pattern in dialogue.text for pattern in ["${", "}}", "function"]
                ):
                    warnings.append(
                        ValidationError(
                            level="warning",
    ...
    def _get_next_custom_id(self) -> int:
        ...
        used_custom_ids = [
            qid
            for qid in self.existing_quest_ids
...
if __name__ == "__main__":
    ...
    test_quest.dialogues.append(Dialogue(text="Hello, adventurer!"))
