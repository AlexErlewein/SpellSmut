...
class QuestDataService:
    ...
    def _load_quest_descriptions(self) -> Dict:
        ...
        if self._quest_descriptions_cache is None:
            ...
            possible_paths = [
                self.project_root / "quest_descriptions_complete.json",
                self.data_dir / "quest_descriptions_complete.json",
                self.data_dir / "quest_maps_and_descriptions.json",
            ]
    ...
    def _load_quest_maps(self) -> Dict:
        ...
        if self._quest_maps_cache is None:
            # Try multiple possible locations
            possible_paths = [
                self.project_root / "quest_maps_and_descriptions.json",
                self.data_dir / "quest_maps_and_descriptions.json",
            ]
    ...
    def _load_quest_rewards(self) -> Dict:
        ...
        if self._quest_rewards_cache is None:
            # First try to load from the complete rewards JSON (with items and money)
            complete_file = self.data_dir / "quest_rewards_complete.json"
            if complete_file.exists():
                with open(complete_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    # Extract the rewards_by_quest_id section
                    self._quest_rewards_cache = data.get("rewards_by_quest_id", {})
                    print(
                        f"[INFO] Loaded {len(self._quest_rewards_cache)} complete quest rewards with items/money"
                    )
            else:
                # Fall back to CSV file
                csv_file = self.data_dir / "FINAL_REWARD_WITH_QUEST_IDS.csv"
                if csv_file.exists():
                    self._quest_rewards_cache = self._load_rewards_from_csv(csv_file)
                else:
                    # Try to load from existing reward mappings JSON
    ...
    def _load_rewards_from_csv(self, csv_file: Path) -> Dict:
        ...
        try:
            with open(csv_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    # Skip if no valid quest ID
                    if (
                        not quest_id
                        or quest_id == "NOT_FOUND"
                        or not quest_id.isdigit()
                    ):
                        continue

                    xp = row.get("XP", "0").strip()
                    try:
                        xp = int(xp)
                    except ValueError:
                        xp = 0
                    ...
                    if xp > 0 or items:
                        rewards[quest_id] = {
                            "xp": xp,
                            "items": items,
                            "flags": [reward_name] if reward_name else [],
                            "gold": 0,
                        }

            print(f"[INFO] Loaded {len(rewards)} quest rewards from CSV")
            return rewards

        except Exception as e:
            print(f"[WARNING] Failed to load rewards from CSV: {e}")
            return self._get_hardcoded_rewards()
    ...
    def get_enhanced_quest_data(
        self, quest_id: int, cff_quest_data: Optional[Dict] = None
    ) -> EnhancedQuestData:
        """
        ...
        if quest_id_str in quest_maps:
            ...
            for map_info in map_data.get("maps", []):
                quest_data.map_locations.append(
                    MapLocation(code=map_info["code"], name=map_info["name"])
                )
        ...
        if quest_id_str in quest_rewards:
            ...
            quest_data.rewards = QuestReward(
                ...
                silver=reward_data.get("silver", 0),
                copper=reward_data.get("copper", 0),
            )

        # Calculate statistics
        quest_data.lua_files_count = len(quest_data.file_references)
        quest_data.total_lua_references = sum(
            f.reference_count for f in quest_data.file_references
        )
    ...
    def get_quest_relationships(
        self, quest_id: int, all_quests: List[Dict]
    ) -> List[QuestRelationship]:
        """
    ...
    def _load_from_cache(self):
        ...
        try:
            ...
            print(
                f"[INFO] Loaded quest data from cache ({len(self._merged_cache or {})} quests)"
            )
            return True
        except Exception as e:
    ...
    def _save_to_cache(self):
        ...
        try:
            ...
            print("[INFO] Saved quest data to cache")
            return True
        except Exception as e:
