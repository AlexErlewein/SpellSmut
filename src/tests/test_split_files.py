...
@pytest.fixture
def icons_dir(tmp_path: Path):
    """Fixture that creates mock split files and returns the icons directory"""
    return create_mock_split_files(tmp_path)
...
def test_find_icon(icons_dir: Path):
    ...
    for category, atlas_num, icon_idx, should_exist in test_cases:
        ...
        if should_exist:
            assert icon is not None, f"Expected to find icon {category}_atlas_{atlas_num}_icon_{icon_idx:03d}"
            print("✓ Found")
            assert icon["category"] == category
            assert icon["atlas_number"] == atlas_num
            assert icon["icon_index"] == icon_idx
        else:
            assert icon is None, f"Expected not to find icon {category}_atlas_{atlas_num}_icon_{icon_idx:03d}"
