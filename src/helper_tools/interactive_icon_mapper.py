...
try:
    from PyQt6.QtWidgets import (
        ...
        QTextEdit, QSplitter, QGroupBox, QGridLayout, QComboBox,
...
class IconMapperWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.mapping_path = self.project_root / "TirganachReloaded" / "data" / "ui_icon_mapping.json"
    ...
    def load_data(self):
        ...
        for item_id_str, icons in sorted(
            self.mapping['item_to_icons'].items(),
    ...
    def load_verified_mappings(self):
        ...
        verified_path = self.project_root / "TirganachReloaded" / "data" / "verified_icon_mappings.json"
    ...
    def filter_items(self):
        ...
        for item in self.item_list:
            # Search filter
            if search_text:
                if search_text not in str(item['id']).lower() and \
                   search_text not in str(item['name']).lower():
                    continue
    ...
    def on_item_selected(self, current: QListWidgetItem, previous: QListWidgetItem):
        ...
        for i, icon_data in enumerate(item['icons'], 1):
            if i in self.icon_selectors:
                ...
                category = 'item' if icon_data['handle'].startswith('ui_item_') else \
                          'spell' if icon_data['handle'].startswith('ui_spell_') else 'other'
                
                # Add all atlases for this category
                atlas_count = 98 if category == 'item' else 18 if category == 'spell' else 0
                ...
                item_id_str = str(item['id'])
                if item_id_str in self.verified_mappings and \
                   str(i) in self.verified_mappings[item_id_str]:
    ...
    def update_icon_preview(self, index: int):
        ...
        # Build icon path
        icon_path = self.icons_root / category / f"atlas_{atlas_num}" / f"icon_{index:03d}.png"
    ...
    def save_current_item(self):
        ...
        self.statusBar().showMessage(f"Saved verification for item {self.current_item_id}")
