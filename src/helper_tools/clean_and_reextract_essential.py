#!/usr/bin/env python3
"""
Clean up and re-extract essential UI icons only.

This script will:
1. Clean up existing extracted icons
2. Re-extract only ITM icons (16x16 grid)
3. Re-extract only ITEM icons (32x32 grid)
4. Re-extract only SPELL icons (64x64 grid)
5. Create unified icon index
"""
...
def main():
    ...
    print("=" * 80)
    print("CLEAN UP AND RE-EXTRACT ESSENTIAL ICONS")
    print("=" * 80)
    ...
    # Only process essential categories
    essential_categories = ["itm", "item", "spell"]

    # Find essential UI DDS files in subdirectories
    all_dds_files = list(extracted_ui.rglob("ui_*.dds"))
    dds_files = [
        f
        for f in all_dds_files
        if any(f"ui_{cat}_" in f.name for cat in essential_categories)
    ]

    print(f"Found {len(dds_files)} essential UI texture files")
    print()
    ...
    for dds in dds_files:
        ...
        if len(parts) >= 2:
            ...
            if match and match.group(1) in essential_categories:
                cat_name = match.group(1)
                cat_num = match.group(2)
                by_category.setdefault(cat_name, []).append((cat_num, dds))

    print(f"Found {len(by_category)} essential categories:")
    ...
    index_data = {
        ...
        "extraction_info": {
            ...
            "categories_processed": essential_categories,
            "description": "Essential icons only (itm, item, spell)",
        },
    }
    ...
    # Print summary
    print("=" * 80)
    print("ESSENTIAL ICONS RE-EXTRACTION SUMMARY")
    print("=" * 80)
    ...
    print("=" * 80)
    print()
    print("✅ ESSENTIAL ICONS CLEAN UP AND RE-EXTRACTION COMPLETE!")
    print()
    print("Essential icons have been re-extracted with latest methods:")
    ...
    print()
    print("Total icons extracted: {:,}".format(stats["icons_extracted"]))
    print()
